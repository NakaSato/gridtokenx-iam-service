...
- **REST** on `IAM_PORT` (bare-metal default 4010): `/api/v1/auth/{register,login,verify,forgot-password,reset-password}` (rate-limited), `/api/v1/users/me[/wallets…]`, `/api/v1/system/config`, `/health`, `/health/ready` (checks Postgres + Redis), `/health/live`, `/metrics` (Prometheus).
